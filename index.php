...
<main role="main">
	...
	<?php
	global $wp_query;
	$taxonomy_obj = $wp_query->tax_query;
	$terms = $taxonomy_obj->queried_terms;
	?>

	<?php if (!empty($terms)) {
		if (is_tax('serial')) {
			// Redirect to taxonomy-serial.php
			get_template_part('templates/taxonomy', 'serial');
			exit;
		} else {
			// Redirect to taxonomy-custom.php
