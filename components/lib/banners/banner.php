...
?>
  ...
  <?php } else { ?>
    <div class="banner gap--20 <?php echo strlen($extra_class) > 0 ? ' ' . $extra_class : ''; ?>">
      <div class="inner">
        <div class="title">
          <h1 class="<?php echo strlen($headline_class) > 0 ? $headline_class : ''; ?>"><?php _e('Free and Open', 'mongabay'); ?></h1>
        </div>
        <?php if (strlen($copy) > 0) { ?>
          <div class="copy">
            <?php _e($copy, 'mongabay'); ?>
        ...
        <?php } ?>
        <a href="https://mongabay.org/about" class="theme--button primary <?php echo strlen($button_class) > 0 ? $button_class : ''; ?>"><?php _e('Learn more', 'mongabay'); ?><span class="icon icon-right"></span>
        </a>
      </div>
    </div>
<?php }
